...
DLSPEED_RESUME_LIMIT	EQU 3
DLSPEED_RESUME_DELAY	EQU 500
...
.SEND_REQUEST
	...
	CALL	RECEIVE_BODY
	JR	NC,.RECEIVE_COMPLETE
	LD	(RECEIVE_RESULT),A
	CALL	PREPARE_RANGE_RETRY
	JR	NC,.CONNECT_ATTEMPT
	LD	A,(RECEIVE_RESULT)
	JP	RECEIVE_ERROR

.RECEIVE_COMPLETE
	CALL	COMMIT_FRAGMENT
	JR	NC,.TRANSFER_COMPLETE
	LD	A,RES_ERROR
	LD	(RECEIVE_RESULT),A
	JP	RECEIVE_ERROR
...
.TRANSFER_COMPLETE
	...
	PRINT	MSG_RECEIVED
	LD	HL,(TOTAL_RECEIVED)
	LD	DE,(TOTAL_RECEIVED+2)
...
.RESULT_READY
	...
	; length. TOTAL_RECEIVED also includes fragments completed before a Range
	; reconnect, so it is the exact application-visible truncation point.
	PRINT	MSG_RECEIVED
	LD	HL,(TOTAL_RECEIVED)
	LD	DE,(TOTAL_RECEIVED+2)
	CALL	TPUT.PRINT_DEC_32
	PRINT	MSG_OF
	LD	HL,(TOTAL_EXPECTED)
	LD	DE,(TOTAL_EXPECTED+2)
	CALL	TPUT.PRINT_DEC_32
	PRINTLN	MSG_BYTES
	RET

; Commit the current HTTP fragment into the end-to-end counters. The first
; response establishes the total length. A resumed response must be exactly
; HTTP 206 and advertise the still-missing byte count; this prevents a server
; which ignored Range and returned 200/full content from inflating the sample.
COMMIT_FRAGMENT
	LD	A,(FRAGMENT_COMMITTED)
	OR	A
	RET	NZ
	LD	A,(DHTTP.CONTENT_LENGTH_SEEN)
	OR	A
	JR	Z,.RANGE_ERROR
	LD	A,(RANGE_ACTIVE)
	OR	A
	JR	NZ,.VALIDATE_RANGE
	LD	HL,DHTTP.CONTENT_LENGTH
	LD	DE,TOTAL_EXPECTED
	LD	BC,4
	LDIR
	JR	.ADD_BODY
.VALIDATE_RANGE
...
.ADD_NEXT
	...
	RET
.RANGE_ERROR
	LD	A,DHTTP_ERR_RANGE
	LD	(DHTTP.ERROR),A
	SCF
	RET

; CF=0 iff resumed Content-Length == TOTAL_EXPECTED - TOTAL_RECEIVED.
RANGE_LENGTH_MATCHES
	LD	HL,(TOTAL_EXPECTED)
	LD	DE,(TOTAL_RECEIVED)
	OR	A
	SBC	HL,DE
	LD	(RANGE_REMAIN),HL
	LD	HL,(TOTAL_EXPECTED+2)
	LD	DE,(TOTAL_RECEIVED+2)
	SBC	HL,DE
	LD	(RANGE_REMAIN+2),HL
	JR	C,.NO
	LD	HL,RANGE_REMAIN
	LD	DE,DHTTP.CONTENT_LENGTH
	LD	B,4
.CMP
	LD	A,(DE)
	CP	(HL)
...
; CF=0 when TOTAL_RECEIVED <= TOTAL_EXPECTED.
TOTAL_NOT_OVER_EXPECTED
	LD	HL,TOTAL_RECEIVED+3
	LD	DE,TOTAL_EXPECTED+3
	LD	B,4
.CMP
	LD	A,(DE)
	CP	(HL)
...
; Retry only clean, non-empty, incomplete transfers. The retry uses an HTTP
; Range request, keeps the original RTC timer and diagnostics running, and is
; bounded so a persistently failing peer still returns a useful error.
PREPARE_RANGE_RETRY
	LD	A,(RECEIVE_RESULT)
	CP	RES_NOT_CONN
	JR	Z,.TRANSPORT_OK
...
.TRANSPORT_OK
	...
	CALL	COMMIT_FRAGMENT
	JR	C,.NO
	CALL	TOTAL_RECEIVED_NONZERO
	JR	Z,.NO
	CALL	TOTAL_BELOW_EXPECTED
	JR	NC,.NO
	LD	A,(RESUME_COUNT)
	CP	DLSPEED_RESUME_LIMIT
	JR	NC,.NO
	CALL	ABORT_SOCKET
	LD	A,(ABORT_STATUS)
	CP	1
...
TOTAL_RECEIVED_NONZERO
	LD	A,(TOTAL_RECEIVED)
	LD	HL,TOTAL_RECEIVED+1
	OR	(HL)
	INC	HL
	OR	(HL)
	INC	HL
...
; CF=1 iff TOTAL_RECEIVED < TOTAL_EXPECTED.
TOTAL_BELOW_EXPECTED
	LD	HL,TOTAL_RECEIVED+3
	LD	DE,TOTAL_EXPECTED+3
	LD	B,4
.CMP
	LD	A,(DE)			; expected byte
	CP	(HL)			; expected - received
	JR	NZ,.DIFF
	DEC	HL
	DEC	DE
	DJNZ	.CMP
	...
	RET
.DIFF
	CCF				; CF=1 iff received < expected
	RET
...
; Append a little-endian 32-bit unsigned integer at DE as decimal. This is a
; request-builder routine only; it never runs inside the measured RX loop.
APPEND_U32_FROM_DE
...
MSG_DIAG_RECONNECTS DB "Reconnects: ",0
...
MSG_RESUMING	DB "Connection interrupted; resuming at ",0
...
MSG_BAD_RANGE	DB "Server rejected or ignored the HTTP Range resume.",0
...
REQ_COLON	DB ":",0
REQ_RANGE	DB 13,10,"Range: bytes=",0
REQ_RANGE_END	DB "-",0
