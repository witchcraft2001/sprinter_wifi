...
tmp_base=${TMPDIR:-/tmp}/sprinter-send-defer-vectors
...
sjasmplus --nologo --fullpath \
  ...
  -I "$repo_root/src/lib" \
  --sym="$sym_file" --raw="$raw_file" "$script_dir/send_defer_vectors.asm"
...
if [ "$result" != 0 ]; then
  echo "ESP_TCP_RX_DEFER receive-defer vector mismatch" >&2
  exit 1
fi

echo "ESP_TCP_RX_DEFER receive-defer vectors: OK"
