...
tmp_base=${TMPDIR:-/tmp}/sprinter-ping-vectors
...
sjasmplus --nologo --fullpath \
  ...
  -I "$repo_root/src/lib" \
  --sym="$sym_file" --raw="$raw_file" "$script_dir/ping_vectors.asm"
...
if [ "$result" != 0 ]; then
  echo "PING vector mismatch (checkpoint $result)" >&2
  exit 1
fi

echo "PING vectors: OK"
