...
tmp_base=${TMPDIR:-/tmp}/sprinter-progress-vectors
...
sjasmplus --nologo --fullpath \
  ...
  -I "$repo_root/src/lib" \
  --sym="$sym_file" --raw="$raw_file" "$script_dir/progress_vectors.asm"
...
if [ "$result" != 0 ]; then
  echo "Progress-line vector mismatch" >&2
  exit 1
fi

echo "Progress line vectors: OK"
