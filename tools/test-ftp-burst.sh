...
test -n "$start_addr"
test -n "$end_addr"
# A single clean 4K receive, the error probes and two 200-tick waits for the
# keyboard service are well below 16M cycles. A regression into the 20000-tick
# idle wait (~260M cycles) fails this bound and the marker.
ticks=$(z88dk-ticks -l 16128 -pc "$start_addr" -end "$end_addr" \
  -counter 16000000 -output "$ram_file" "$raw_file")
marker=$(od -An -tu1 -j 49153 -N 1 "$ram_file" | tr -d ' ')
result=$(od -An -tu1 -j 49152 -N 1 "$ram_file" | tr -d ' ')
if [ "$marker" != 165 ] || [ "$result" != 0 ]; then
...
echo "FTP 1000-tick idle: $ticks cycles, only 4 keyboard RTS pauses (21 MHz)"

app_idle_addr=$(awk '/^BENCH_APP_IDLE:/ {sub(/^0x0*/, "", $3); print $3}' "$sym_file")
test -n "$app_idle_addr"
ticks=$(z88dk-ticks -l 16128 -pc "$app_idle_addr" -end "$end_addr" \
  -counter 24000000 -output "$ram_file" "$raw_file")
marker=$(od -An -tu1 -j 49153 -N 1 "$ram_file" | tr -d ' ')
result=$(od -An -tu1 -j 49152 -N 1 "$ram_file" | tr -d ' ')
if [ "$marker" != 165 ] || [ "$result" != 0 ]; then
  echo "Application stable-RTS idle vector $result failed ($ticks ticks, marker $marker)" >&2
  exit 1
fi
echo "Application 300-tick idle: $ticks cycles, RTS never dropped, ISA kept mapped"
